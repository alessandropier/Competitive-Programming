...
## What does **not** work
The easiest idea to come to mind would be a simple for loop
```cpp
int sum = 0;
int val = 0;
...
1. **Integer Overflow:** Storing the sum in a standard 32-bit `int` will cause an overflow. Given the constraints ($50,000,000 \times 67,108,863$), the maximum potential sum requires up to 52 bits. Using a 64-bit `long long` is mandatory.
2. **Excessive System Calls:** Standard functions like `std::cin` or `scanf` read data in small fragments. Processing 50 million numbers triggers thousands of System Calls to request more data from the OS, wasting a massive amount of CPU cycles.
3. **Cache Misses:** Because the data is read in fragmented chunks from the RAM, the CPU goes constantly back and forth to the RAM to fetch single numbers from the input file. The CPU suffers from multiple **cache misses**, forcing it to wait for the slow main memory instead of fetching data instantly from the L1/L2 cache.
4. **The `sync_with_stdio` Illusion:** Even adding the classic one-liner optimization:
```cpp
    ios_base::sync_with_stdio(false);
```
will **not** save this solution because it does not bypass the problem explained above.
...
* **Drastically reduce RAM access cycles:** Instead of querying the RAM for every single value, the goal is to fetch data in **big chunks** using a custom _memory_ _buffer_.
* **Maximize CPU Cache efficiency:** Keep the active values strictly within the boundaries of the L1/L2 CPU cache. This allows the processor to fetch subsequent bytes in 1-5 clock cycles rather than waiting for the 150-200 cycles required by standard RAM reads.
* **Bypass heavyweight parsing:** Avoid all unnecessary features of standard library parsers (_like timezone, floating-point checks, and region locales_). The goal is to perform a direct, lightweight math conversion (`val * 10 + (...)`) executing minimal assembly operations per digit.
* **Achieve $\mathcal{O}(1)$ Space Complexity:** Avoid loading the full array of 50 million integers into memory. By accumulating the sum **while** processing the buffer, the application maintains a near-zero memory footprint.
...
At this level, the code does not process the input as individual numerical types, but rather as a continuous text stream. These are the implemented steps:
1. **Buffer Loading:** The application pulls a huge block of text from the file straight into our 64 KB _"cache-friendly"_ buffer.
...
3. **Sign & Digits:** Moving sequentially using the pointers, the code isolates the negative sign (if present) and extracts the numeric characters.
4. **Accumulation:** The characters are converted into a 64-bit integer and added to the total sum, maintaining an optimal $O(1)$ memory footprint.
...
Below there is a detailed breakdown of the architectural choices, memory layouts, and micro-optimizations:
...
* **Pointers:** The code fills the 64 KB cache buffer using `fread`. Then it initializes a pointer (`ptr`) to iterate through the text characters sequentially.
...
* **Why 32 Characters?** A single number under the problem constraints can be up to 9–10 characters long (including the minus sign). By checking if we have fewer than 32 characters left (`ptr > end_ptr - 32`), we ensure there is *always* enough contiguous string space to fully parse the next number without facing an abrupt End-of-Buffer error. _(note: 32 is not the only valid threshold. Any value greater than the maximum number of digits a number can have under the problem constraints would work. For example, 11 would work because 11 > 10.")_
* **Branch Prediction via `__builtin_expect`:** Refilling the buffer occurs very rarely (only once every 64 KB of processed text). The compiler macro `__builtin_expect(..., 0)` forces the CPU to optimize for the _most common path_ (the parsing loop), treating the reload branch as highly unlikely.
